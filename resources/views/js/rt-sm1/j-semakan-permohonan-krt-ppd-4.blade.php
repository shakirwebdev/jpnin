...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		$('#spkp4_krt_status_bagunan_id').val("{{$profile_krt->krt_status_bagunan_id}}");

		if($('#spkp4_krt_status_bagunan_id').val() == '1'){
			$(".for_binaan").css("display", "block");
			$(".for_tumpang").css("display", "none");
		} else if ($('#spkp4_krt_status_bagunan_id').val() == '2') {
			$(".for_binaan").css("display", "none");
			$(".for_tumpang").css("display", "block");
		} else if ($('#spkp4_krt_status_bagunan_id').val() == '3') {
			$(".for_binaan").css("display", "none");
			$(".for_tumpang").css("display", "none");
		}
		...
		$('#btn_next').click(function(){
			window.location.href = "{{route('rt-sm1.semakan_permohonan_krt_ppd_5','')}}"+"/"+{{$profile_krt->id}};
		});
