...
<script type="text/javascript">    
    ...
    $(document).ready( function () {
        ...
        var senarai_perancangan_rondaan_config = {
            routes: {
                senarai_perancangan_rondaan_url: "/rt/sm15/jana-jadual-rondaan-ppd"
            }
        };

        $("#jjrpd_krt_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#jjrpd_srs_id').find('option').remove();
			$('#jjrpd_srs_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					...
					success: function (data) {
						$('#jjrpd_srs_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
        ...
        $("#jjrpd_srs_id").on( 'change', function () {
