...
<script type="text/javascript">    
    ...
    $(document).ready( function () {
        ...
        var senarai_perancangan_rondaan_config = {
            routes: {
                senarai_perancangan_rondaan_url: "/rt/sm15/jana-jadual-rondaan-ppn"
            }
        };

        $("#jjrpn_daerah_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#jjrpn_krt_id').find('option').remove();
			$('#jjrpn_krt_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					...
					success: function (data) {
						$('#jjrpn_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#jjrpn_krt_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#jjrpn_srs_id').find('option').remove();
			$('#jjrpn_srs_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					url: senarai_perancangan_rondaan_config.routes.senarai_perancangan_rondaan_url,
					...
					success: function (data) {
						$('#jjrpn_srs_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
						$.each(data,function(key, obj) 
						{
							$('#jjrpn_srs_id')
							.append($('<option>')
		...
		$("#jjrpn_srs_id").on( 'change', function () {
            senari_perancangan_rondaan.search( $(this).val() ).draw();
        });
