...
<script type="text/javascript">    
    ...
    $(document).ready( function () {
		...
		var perancangan_aktiviti_config = {
			routes: {
				perancangan_aktiviti_url: "/rt/sm6/pengesahan-perancangan-aktiviti-ppd-1/{{$perancangan_aktiviti->id}}"
			}
		};
    ...
    //my custom script
        var update_perancangan_aktiviti_config = {
            routes: {
                update_perancangan_aktiviti_url: "{{ route('rt-sm6.post_perancangan_aktiviti_krt_ppd') }}",
            }
        };

    /* Button Seterusnya */
        // $(document).on('submit', '#form_ppap1', function(event){    
        //     event.preventDefault();
        //     $('#btn_next').html("<i class=\"fa fa-circle-o-notch fa-spin mr-10\"></i>Sila tunggu..");
        //     $('#btn_next').prop('disabled', true);
        //     var data   = $("#form_ppap1").serialize();
        //     var action = $('#post_perancangan_aktiviti_krt_ppd').val();
        //     var btn_text;
        //     url = update_perancangan_aktiviti_config.routes.update_perancangan_aktiviti_url;
        //     type = "POST";
        ...
        //         $('[name=ppap1_bahagian_id]').removeClass("is-invalid");
                
        //         if(response.errors){
        //             $.each(response.errors, function(index, error){
        //                 if(index == 'ppap1_bahagian_id') {
        //                     $('[name=ppap1_bahagian_id]').addClass("is-invalid");
        //                     $('.error_ppap1_bahagian_id').html(error);
        //                 }
