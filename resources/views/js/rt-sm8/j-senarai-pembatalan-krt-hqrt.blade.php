...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_pembatalan_krt_config = {
			routes: {
				senarai_pembatalan_krt_url: "/rt/sm8/senarai-pembatalan-krt-hqrt"
			}
		};
        
        $("#spkhq_state_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#spkhq_daerah_id').find('option').remove();
			$('#spkhq_daerah_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					...
					success: function (data) {
						$('#spkhq_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
        ...
        $("#spkhq_daerah_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#spkhq_krt_id').find('option').remove();
			$('#spkhq_krt_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					url: senarai_pembatalan_krt_config.routes.senarai_pembatalan_krt_url,
					...
					success: function (data) {
						$('#spkhq_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
						$.each(data,function(key, obj) 
						{
							$('#spkhq_krt_id')
							.append($('<option>')
		...
		$("#spkhq_krt_id").on( 'change', function () {
