...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_permohonan_pembatalan_krt_config = {
			routes: {
				senarai_permohonan_pembatalan_krt_url: "/rt/sm8/sokongan-pembatalan-krt"
			}
		};
        ...
        var senari_permohonan_pembatalan_krt = $('#senari_permohonan_pembatalan_krt').DataTable( {
			...
			"aoColumnDefs":[{          
                ...
                "mRender": function ( value, type, full )  {
                    if (full.status == '4') {
                        button_a = '<button type="button" class="btn btn-icon" title="Sokongan Permohonan Pembatalan KRT" id="edit-jpnin" onclick="sokongan_permohonan_pembatalan_krt(\'' + full.id + '\');"><i class="fa fa-edit"></i></button>';
					    return button_a;
                    } else {
                        button_b = '';
                        return button_b;
    ...
    function sokongan_permohonan_pembatalan_krt(id){
        window.location.href = "{{ route('rt-sm8.sokongan_pembatalan_krt_1','') }}"+"/"+id;
    }
