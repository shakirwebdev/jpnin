...
<script type="text/javascript">    
	...
	$(document).ready( function () {
        ...
        /* Maklumat Pembatalan Krt */
			...
			$('#spk_disokong_note').on('keyup keypress', function(e) {
		...
		/* Button */
			$('#btn_back').click(function(){
                window.location.href = "{{route('rt-sm8.sokongan_pembatalan_krt')}}";
            });
    ...
    /* action submit */
        //my custom script
        var sokongan_pembatalan_ket_config = {
            routes: {
                sokongan_pembatalan_krt_url: "{{ route('rt-sm8.post_sokongan_pembatalan_krt') }}",
            }
        };
        ...
        $(document).on('click', '#btn_submit', function(event){    
            ...
            var action = $('#post_sokongan_pembatalan_krt').val();
            var btn_text;
            if (action == 'edit') {
                url = sokongan_pembatalan_ket_config.routes.sokongan_pembatalan_krt_url;
                type = "POST";
            ...
            $.ajax({
                ...
                $('[name=spk_disokong_note]').removeClass("is-invalid");
            ...
            if(response.errors){
                $.each(response.errors, function(index, error){
                    ...
                    if(index == 'spk_disokong_note') {
                        $('[name=spk_disokong_note]').addClass("is-invalid");
                        $('.error_spk_disokong_note').html(error);
                    }
                });
