...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var senarai_student_tp_ppd_config = {
			routes: {
				senarai_student_tp_ppd_url: "/rt/sm27/senarai-student-tp-ppd"
			}
		};  

        $("#sstpd_tabika_id").on( 'change', function () {
    	...
    	var list_mohon_masuk_tabika = $('#list_mohon_masuk_tabika').DataTable( {
			...
			"aoColumnDefs":[{          
                ...
                "mRender": function ( value, type, full )  {
                    if (full.status == 1) {
                        button_a = '<button type="button" class="btn btn-icon" title="Paparan Profile Pelajar Tabika Perpaduan" onclick="paparan_student_tp_ppd(\'' + full.id + '\');"><i class="fa fa-search"></i></button>';
					    button_b = '<button type="button" class="btn btn-icon" title="Paparan Permohonan"  data-id="" onclick="jana_surat_pemakluman(\'' + full.id + '\');"><i class="fa fa-print"></i></button>';
    ...
    function paparan_student_tp_ppd(id){
		window.location.href = "{{ route('rt-sm27.student_tp_ppd','') }}"+"/"+id;
	}

    function jana_surat_pemakluman(id){
