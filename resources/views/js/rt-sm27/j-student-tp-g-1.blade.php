...
<script type="text/javascript">  
	...
	$(document).ready( function () {
		...
		/* Bahagian B : Maklumat Ibu */
			$('#stg_ibu_nama').val("{{$tbk_student->ibu_nama}}");
			$('#stg_ibu_pekerjaan').val("{{$tbk_student->ibu_pekerjaan}}");
			$('#stg_ibu_profession_id').val("{{$tbk_student->ibu_profession_id}}");
			$('#stg_ibu_pendapatan').val("{{$tbk_student->ibu_pendapatan}}");
			$('#stg_ibu_kerakyatan_id').val("{{$tbk_student->ibu_kerakyatan_id}}");
			$('#stg_ibu_jumlah_pendapatan').val("{{$tbk_student->ibu_jumlah_pendapatan}}");
			$('#stg_ibu_jumlah_pendapatan_lain').val("{{$tbk_student->ibu_jumlah_pendapatan_lain}}");
			$('#stg_ibu_ic').val("{{$tbk_student->ibu_ic}}");
			$('#stg_ibu_alamat_office').val("{{$tbk_student->ibu_alamat_office}}");
			$('#stg_ibu_phone_office').val("{{$tbk_student->ibu_phone_office}}");
			$('#stg_ibu_phone').val("{{$tbk_student->ibu_phone}}");
			$('#stg_ibu_phone_rumah').val("{{$tbk_student->ibu_phone_rumah}}");
			$('#stg_ibu_bil_anak').val("{{$tbk_student->ibu_bil_anak}}");
			$('#stg_ibu_hubungan_student').val("{{$tbk_student->ibu_hubungan_student}}");
			$('#stg_ibu_pilihan').val("{{$tbk_student->ibu_pilihan}}");
			$('#stg_ibu_bil_anak_sekolah').val("{{$tbk_student->ibu_bil_anak_sekolah}}");
			$('#stg_ibu_tabika_lain').val("{{$tbk_student->ibu_tabika_lain}}");
            $('#stg_student_id').val("{{$tbk_student->id}}");
			$('#stg_1_student_id').val("{{$tbk_student->id}}");
		...
		/* Button */
			$('#btn_back').click(function(){
				window.location.href = "{{route('rt-sm27.student_tp_g','')}}"+"/"+"{{$tbk_student->id}}";
			});
	});
