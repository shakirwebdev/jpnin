...
<script type="text/javascript">  
	...
	$(document).ready( function () {
		...
		/* Bahagian B : Maklumat Ibu */
			$('#stpd_ibu_nama').val("{{$tbk_student->ibu_nama}}");
			$('#stpd_ibu_pekerjaan').val("{{$tbk_student->ibu_pekerjaan}}");
			$('#stpd_ibu_profession_id').val("{{$tbk_student->ibu_profession_id}}");
			$('#stpd_ibu_pendapatan').val("{{$tbk_student->ibu_pendapatan}}");
			$('#stpd_ibu_kerakyatan_id').val("{{$tbk_student->ibu_kerakyatan_id}}");
			$('#stpd_ibu_jumlah_pendapatan').val("{{$tbk_student->ibu_jumlah_pendapatan}}");
			$('#stpd_ibu_jumlah_pendapatan_lain').val("{{$tbk_student->ibu_jumlah_pendapatan_lain}}");
			$('#stpd_ibu_ic').val("{{$tbk_student->ibu_ic}}");
			$('#stpd_ibu_alamat_office').val("{{$tbk_student->ibu_alamat_office}}");
			$('#stpd_ibu_phone_office').val("{{$tbk_student->ibu_phone_office}}");
			$('#stpd_ibu_phone').val("{{$tbk_student->ibu_phone}}");
			$('#stpd_ibu_phone_rumah').val("{{$tbk_student->ibu_phone_rumah}}");
			$('#stpd_ibu_bil_anak').val("{{$tbk_student->ibu_bil_anak}}");
			$('#stpd_ibu_hubungan_student').val("{{$tbk_student->ibu_hubungan_student}}");
			$('#stpd_ibu_pilihan').val("{{$tbk_student->ibu_pilihan}}");
			$('#stpd_ibu_bil_anak_sekolah').val("{{$tbk_student->ibu_bil_anak_sekolah}}");
			$('#stpd_ibu_tabika_lain').val("{{$tbk_student->ibu_tabika_lain}}");
            $('#stpd_student_id').val("{{$tbk_student->id}}");
			$('#stpd_1_student_id').val("{{$tbk_student->id}}");
		...
		/* Button */
			$('#btn_back').click(function(){
				window.location.href = "{{route('rt-sm27.student_tp_ppd','')}}"+"/"+"{{$tbk_student->id}}";
			});
	});
