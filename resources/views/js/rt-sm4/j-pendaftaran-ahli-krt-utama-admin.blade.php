...
<script type="text/javascript">    
    ...
    $(document).ready( function () {
		...
		var senarai_ahli_krt_config = {
			routes: {
				senarai_senarai_ahli_krt_url: "/rt/sm4/pendaftaran-ahli-krt-utama-admin"
			}
		};

        $("#pakuas_negeri_id").on( 'change', function () {
            var value = $(this).find('option:selected').val();
            var selectedIndex = $(this).find('option:selected').index();
            $('#pakuas_daerah_id').find('option').remove();
            $('#pakuas_daerah_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#pakuas_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
        ...
        $("#pakuas_daerah_id").on( 'change', function () {
            var value = $(this).find('option:selected').val();
            var selectedIndex = $(this).find('option:selected').index();
            $('#pakuas_krt_id').find('option').remove();
            $('#pakuas_krt_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: senarai_ahli_krt_config.routes.senarai_senarai_ahli_krt_url,
                    ...
                    success: function (data) {
                        $('#pakuas_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#pakuas_krt_id')
                            .append($('<option>')
