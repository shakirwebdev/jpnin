...
<script type="text/javascript">    
    $(document).ready( function () {
		...
		var senarai_minit_mesyuarat_config = {
			routes: {
				senarai_minit_mesyuarat_url: "/rt/sm5/pengesahan-minit-mesyuarat-rt"
			}
		};
		...
		var senarai_minit_mesyuarat_table = $('#senarai_minit_mesyuarat_table').DataTable( {
			...
			"aoColumnDefs":[{          
				...
				sClass: 'text-center',
                "mRender": function ( value, type, full )  {
                    if (full.mesyuarat_status == '3') {
                        button_a = '<button type="button" class="btn btn-icon" title="Kemaskini pendaftaran ahli KRT" onclick="pengesahan_minit_mesyuarat_rt_ppd(\'' + full.id + '\');"><i class="fa fa-edit"></i></button>';
					    return button_a;
                    } else {
                        button_b = '';
                        return button_b;
	...
	function pengesahan_minit_mesyuarat_rt_ppd(id){
        window.location.href = "{{ route('rt-sm5.pengesahan_minit_mesyuarat_rt_ppd','') }}"+"/"+id;
    }
