...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		//my custom script
			...
			routes: {
				senarai_senarai_ahli_peronda_config_url: "/rt/sm13/senarai-pendaftaran-ahli-peronda-srs"
			}
		};
		...
		var senarai_ahli_peronda_srs_table = $('#senarai_ahli_peronda_srs_table').DataTable( {
			...
			"aoColumnDefs":[{          
                ...
                "mRender": function ( value, type, full )  {
                    if (full.status == '3') {
                        button_a = '<button type="button" class="btn btn-icon" title="Kemaskini Pendaftaran Ahli Peronda SRS" onclick="kemaskini_permohonan_srs(\'' + full.id + '\');"><i class="fa fa-edit"></i></button>';
					    return button_a;
	...
	function kemaskini_permohonan_srs(id){
		window.location.href = "{{ route('rt-sm13.pendaftaran_ahli_peronda_srs','') }}"+"/"+id;
	}
