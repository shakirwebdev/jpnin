...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		/* Maklumat Kawasan Krt */
            $('#apspd1_nama_krt').html("{{$srs_ahli_peronda->nama_krt}}");
            $('#apspd1_alamat_krt').html("{{$srs_ahli_peronda->alamat_krt}}");
            $('#apspd1_negeri_krt').html("{{$srs_ahli_peronda->negeri_krt}}");
            $('#apspd1_parlimen_krt').html("{{$srs_ahli_peronda->daerah_krt}}");
            $('#apspd1_pbt_krt').html("{{$srs_ahli_peronda->parlimen_krt}}");
            $('#apspd1_daerah_krt').html("{{$srs_ahli_peronda->dun_krt}}");
            $('#apspd1_dun_krt').html("{{$srs_ahli_peronda->pbt_krt}}");

        /* Maklumat Status Semakan */
            $('#apspd1_ahli_peronda_id').val("{{$srs_ahli_peronda->id}}");
		...
		$('#btn_back').click(function(){
			window.location.href = "{{route('rt-sm13.senarai_ahli_peronda_srs_ppd_1','')}}"+"/"+{{$srs_ahli_peronda->id}};
		});
	...
	function getPekerjaan(id) {
		...
		if ($('#chkp_1' + id).is(':checked')) {
			// alert('checked');
			var apspd1_srs_ahli_peronda_id = $('#apspd1_srs_ahli_peronda_id').val();
			...
			$.ajax({
				...
				data: {
						"_token": "{{ csrf_token() }}",
						"apspd1_srs_ahli_peronda_id": apspd1_srs_ahli_peronda_id,
