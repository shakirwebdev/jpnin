...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_ajk_krt_config = {
			routes: {
				laporan_ajk_krt_url: "/rt/sm30/laporan-ajk-krt-umur-ppd"
			}
		}; 

        $("#lakupd_parlimen_id").on( 'change', function () {
			senarai_ajk_krt.column('3:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lakupd_dun_id').find('option').remove();
            $('#lakupd_dun_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lakupd_dun_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
