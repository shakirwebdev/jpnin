...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_maklumat_asas_config = {
			routes: {
				laporan_maklumat_asas_url: "/rt/sm30/laporan-maklumat-asas-krt-ppd"
			}
		}; 

		$("#lmakpd_parlimen_id").on( 'change', function () {
			laporan_maklumat_asas_rt.search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lmakpd_dun_id').find('option').remove();
            $('#lmakpd_dun_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lmakpd_dun_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#lmakpd_dun_id").on( 'change', function () {
			laporan_maklumat_asas_rt.search( $(this).val() ).draw();
		});
