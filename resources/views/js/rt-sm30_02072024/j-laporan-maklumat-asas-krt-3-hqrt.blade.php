...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_maklumat_asas_config = {
			routes: {
				laporan_maklumat_asas_url: "/rt/sm30/laporan-maklumat-asas-krt-3-hqrt"
			}
		}; 

		$("#lmak3hq_state_id").on( 'change', function () {
			laporan_maklumat_asas_rt_3.column('1:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lmak3hq_daerah_id').find('option').remove();
            $('#lmak3hq_daerah_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lmak3hq_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#lmak3hq_daerah_id").on( 'change', function () {
			laporan_maklumat_asas_rt_3.column('2:visible').search( $(this).val() ).draw();
		});
