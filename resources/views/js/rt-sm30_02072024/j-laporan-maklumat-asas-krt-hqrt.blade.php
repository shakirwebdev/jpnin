...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_maklumat_asas_config = {
			routes: {
				laporan_maklumat_asas_url: "/rt/sm30/laporan-maklumat-asas-krt-hqrt"
			}
		}; 

		$("#lmakhq_state_id").on( 'change', function () {
			laporan_maklumat_asas_rt.search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lmakhq_daerah_id').find('option').remove();
            $('#lmakhq_daerah_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lmakhq_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#lmakhq_state_id").on( 'change', function () {
			...
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lmakhq_pbt_id').find('option').remove();
            $('#lmakhq_pbt_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: laporan_maklumat_asas_config.routes.laporan_maklumat_asas_url,
                    ...
                    success: function (data) {
                        $('#lmakhq_pbt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#lmakhq_pbt_id')
                            .append($('<option>')
		...
		$("#lmakhq_state_id").on( 'change', function () {
			...
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lmakhq_parlimen_id').find('option').remove();
            $('#lmakhq_parlimen_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: laporan_maklumat_asas_config.routes.laporan_maklumat_asas_url,
                    ...
                    success: function (data) {
                        $('#lmakhq_parlimen_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#lmakhq_parlimen_id')
                            .append($('<option>')
		...
		$("#lmakhq_parlimen_id").on( 'change', function () {
			laporan_maklumat_asas_rt.search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lmakhq_dun_id').find('option').remove();
            $('#lmakhq_dun_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: laporan_maklumat_asas_config.routes.laporan_maklumat_asas_url,
                    ...
                    success: function (data) {
                        $('#lmakhq_dun_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#lmakhq_dun_id')
                            .append($('<option>')
		...
		$("#lmakhq_daerah_id").on( 'change', function () {
			laporan_maklumat_asas_rt.search( $(this).val() ).draw();
		});

		$("#lmakhq_pbt_id").on( 'change', function () {
			laporan_maklumat_asas_rt.search( $(this).val() ).draw();
		});

		$("#lmakhq_dun_id").on( 'change', function () {
			laporan_maklumat_asas_rt.search( $(this).val() ).draw();
		});
