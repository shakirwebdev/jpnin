...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_permohonan_projek_ekonomi_config = {
			routes: {
				senarai_permohonan_projek_ekonomi_url: "/rt/sm10/semakan-projek-ekonomi-krt"
			}
		};
		...
		var senarai_permohonan_projek_ekonomi = $('#senarai_permohonan_projek_ekonomi').DataTable( {
			...
			"aoColumnDefs":[{          
                ...
                "mRender": function ( value, type, full )  {
                    if (full.status == '4') {
                        button_a = '<button type="button" class="btn btn-icon" title="Semakan Permohonan Sejiwa" onclick="semakan_permohonan_projek_ekonomi(\'' + full.id + '\');"><i class="fa fa-edit"></i></button>';
					    return button_a;
                    } else {
                        button_b = '';
                        return button_b;
	...
	function semakan_permohonan_projek_ekonomi(id){
		window.location.href = "{{ route('rt-sm10.semakan_projek_ekonomi_krt_1','') }}"+"/"+id;
	}
