...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_pelaksanaan_projek_ekonomi_st_config = {
			routes: {
				senarai_pelaksanaan_projek_ekonomi_st_url: "/rt/sm10/semakan-projek-ekonomi-st-krt"
			}
		};
		...
		var senarai_pelaksanaan_projek_ekonomi_st = $('#senarai_pelaksanaan_projek_ekonomi_st').DataTable( {
			...
			"aoColumnDefs":[{          
                ...
                "mRender": function ( value, type, full )  {
                    if (full.status == '4') {
                        button_a = '<button type="button" class="btn btn-icon" title="Semakan Pelaksanaan Projek Ekonomi" onclick="semakan_pelaksanaan_projek_ekonomi(\'' + full.id + '\');"><i class="fa fa-edit"></i></button>';
					    return button_a;
                    } else {
                        button_b = '';
                        return button_b;
	...
	function semakan_pelaksanaan_projek_ekonomi(id){
		window.location.href = "{{ route('rt-sm10.semakan_projek_ekonomi_st_krt_1','') }}"+"/"+id;
	}
