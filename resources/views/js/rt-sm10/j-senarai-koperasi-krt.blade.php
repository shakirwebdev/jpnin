...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_koperasi_config = {
			routes: {
				senarai_koperasi_url: "/rt/sm10/senarai-koperasi-krt"
			}
		};
		...
		var senarai_koperasi = $('#senarai_koperasi').DataTable( {
            ...
            rowCallback: function(nRow, aData, index) {
                var info = senarai_koperasi.page.info();
			...
			"aoColumnDefs":[{          
                ...
                "mRender": function ( value, type, full )  {
                    if (full.status == '1') {
                        button_a = '<button type="button" class="btn btn-icon" title="Paparan Koperasi" onclick="paparan_koperasi(\'' + full.id + '\');"><i class="fa fa-edit"></i></button>';
					    return button_a;
                    } else {
                        button_b = '';
                        return button_b;
	...
	function paparan_koperasi(id){
		window.location.href = "{{ route('rt-sm10.senarai_koperasi_krt_1','') }}"+"/"+id;
	}
