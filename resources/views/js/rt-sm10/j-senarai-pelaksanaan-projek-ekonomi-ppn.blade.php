...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_pelaksanaan_projek_ekonomi_st_config = {
			routes: {
				senarai_pelaksanaan_projek_ekonomi_st_url: "/rt/sm10/senarai-pelaksanaan-projek-ekonomi-ppn"
			}
		};

        $("#sppepn_daerah_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#sppepn_krt_id').find('option').remove();
			$('#sppepn_krt_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					...
					success: function (data) {
						$('#sppepn_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
	...
	function paparan_pelaksanaan_projek_ekonomi(id){
		window.location.href = "{{ route('rt-sm10.senarai_pelaksanaan_projek_ekonomi_ppn_1','') }}"+"/"+id;
	}
