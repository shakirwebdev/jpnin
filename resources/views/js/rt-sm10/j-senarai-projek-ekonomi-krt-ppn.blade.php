...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_projek_ekonomi_config = {
			routes: {
				senarai_projek_ekonomi_url: "/rt/sm10/senarai-projek-ekonomi-krt-ppn"
			}
		};

        $("#spepn_daerah_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#spepn_krt_id').find('option').remove();
			$('#spepn_krt_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					...
					success: function (data) {
						$('#spepn_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#spepn_krt_id").on( 'change', function () {
	...
	function paparan_projek_ekonomi(id){
		window.location.href = "{{ route('rt-sm10.senarai_projek_ekonomi_krt_ppn_1','') }}"+"/"+id;
	}
