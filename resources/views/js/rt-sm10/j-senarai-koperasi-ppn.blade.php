...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_koperasi_config = {
			routes: {
				senarai_koperasi_url: "/rt/sm10/senarai-koperasi-ppn"
			}
		};

        $("#skpn_daerah_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#skpn_krt_id').find('option').remove();
			$('#skpn_krt_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					...
					success: function (data) {
						$('#skpn_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#skpn_krt_id").on( 'change', function () {
	...
	function paparan_koperasi(id){
		window.location.href = "{{ route('rt-sm10.senarai_koperasi_ppn_1','') }}"+"/"+id;
	}
