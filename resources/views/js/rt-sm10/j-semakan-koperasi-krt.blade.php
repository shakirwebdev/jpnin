...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_permohonan_koperasi_config = {
			routes: {
				senarai_permohonan_koperasi_url: "/rt/sm10/semakan-koperasi-krt"
			}
		};
		...
		var senarai_permohonan_koperasi = $('#senarai_permohonan_koperasi').DataTable( {
			...
			"aoColumnDefs":[{          
                ...
                "mRender": function ( value, type, full )  {
                    if (full.status == '4') {
                        button_a = '<button type="button" class="btn btn-icon" title="Semakan Permohonan Koperasi" onclick="semakan_permohonan_koperasi(\'' + full.id + '\');"><i class="fa fa-edit"></i></button>';
					    return button_a;
                    } else {
                        button_b = '';
                        return button_b;
	...
	function semakan_permohonan_koperasi(id){
		window.location.href = "{{ route('rt-sm10.semakan_koperasi_krt_1','') }}"+"/"+id;
	}
