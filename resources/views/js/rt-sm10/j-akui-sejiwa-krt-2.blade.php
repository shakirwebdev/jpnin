...
<script type="text/javascript">    
	...
	$(document).ready( function () {
        ...
        /* Maklumat Status Memperakui */
            $('#ask2_diakui_note').on('keyup keypress', function(e) {
        ...
        /* Button */
            $('#btn_back').click(function(){
                window.location.href = "{{ route('rt-sm10.akui_sejiwa_krt_1','') }}"+"/"+"{{$sejiwa->id}}";
            });
    ...
    /* action submit */
        //my custom script
        var akui_profil_sejiwa_krt_config = {
            routes: {
                akui_profil_sejiwa_krt_url: "{{ route('rt-sm10.post_akui_profile_sejiwa') }}",
            }
        };

        $(document).on('submit', '#form_ask2', function(event){    
            event.preventDefault();
            $('#btn_submit').html("<i class=\"fa fa-circle-o-notch fa-spin mr-10\"></i>Sila tunggu..");
            $('#btn_submit').prop('disabled', true);
            var data = $("#form_ask2").serialize();
            var action = $('#post_akui_profile_sejiwa').val();
            var btn_text;
            if (action == 'edit') {
                url = akui_profil_sejiwa_krt_config.routes.akui_profil_sejiwa_krt_url;
                type = "POST";
            ...
            $.ajax({
                ...
                $('[name=ask2_status]').removeClass("is-invalid");
                $('[name=ask2_diakui_note]').removeClass("is-invalid");
            
            if(response.errors){
                $.each(response.errors, function(index, error){
                    if(index == 'ask2_status') {
                        $('[name=ask2_status]').addClass("is-invalid");
                        $('.error_ask2_status').html(error);
                    }

                    if(index == 'ask2_diakui_note') {
                        $('[name=ask2_diakui_note]').addClass("is-invalid");
                        $('.error_ask2_diakui_note').html(error);
                    }
                });
                ...
                $('#btn_submit').html(btn_text);
                    $('#btn_submit').prop('disabled', false); 
                    window.location.href = "{{route('rt-sm10.akui_sejiwa_krt')}}";
                }
            });
        });
