...
<script type="text/javascript">    
	...
	$(document).ready( function () {
        ...
        /* Maklumat Status Memperakui */
            $('#apsupn_diakui_note').on('keyup keypress', function(e) {
        ...
        $('#btn_back').click(function(){
			window.location.href = "{{ route('rt-sm10.akui_profil_skuad_uniti_krt') }}";
		});
    ...
    /* action submit */
        //my custom script
        var akui_profil_skuad_uniti_krt_config = {
            routes: {
                akui_profil_skuad_uniti_krt_url: "{{ route('rt-sm10.post_akui_profile_skuad_uniti') }}",
            }
        };

        $(document).on('submit', '#form_apsupn', function(event){    
            event.preventDefault();
            $('#btn_submit').html("<i class=\"fa fa-circle-o-notch fa-spin mr-10\"></i>Sila tunggu..");
            $('#btn_submit').prop('disabled', true);
            var data = $("#form_apsupn").serialize();
            var action = $('#post_akui_profile_skuad_uniti').val();
            var btn_text;
            if (action == 'edit') {
                url = akui_profil_skuad_uniti_krt_config.routes.akui_profil_skuad_uniti_krt_url;
                type = "POST";
            ...
            $.ajax({
                ...
                $('[name=apsupn_status]').removeClass("is-invalid");
                $('[name=apsupn_disemak_note]').removeClass("is-invalid");
            
            if(response.errors){
                $.each(response.errors, function(index, error){
                    if(index == 'apsupn_status') {
                        $('[name=apsupn_status]').addClass("is-invalid");
                        $('.error_apsupn_status').html(error);
                    }

                    if(index == 'apsupn_diakui_note') {
                        $('[name=apsupn_diakui_note]').addClass("is-invalid");
                        $('.error_apsupn_diakui_note').html(error);
                    }
                });
