...
<script type="text/javascript">    
	...
	$(document).ready( function () {
        ...
        /* Maklumat Pengesahan */
            $('#pilkk_koperasi_krt_id').val("{{$isu_lokasi_kk->id}}");
        ...
        /* Button */
            $('#btn_back').click(function(){
                window.location.href = "{{ route('rt-sm10.pengesahan_isu_lokasi_kanta_komuniti') }}";
            });
     ...
     /* action submit */
        //my custom script
        var pengesahan_isu_lokasi_kk_config = {
            routes: {
                pengesahan_isu_lokasi_kk_url: "{{ route('rt-sm10.post_pengesahan_isu_lokasi_kanta_komuniti') }}",
            }
        };

        $(document).on('submit', '#form_pilkk', function(event){    
            event.preventDefault();
            $('#btn_submit').html("<i class=\"fa fa-circle-o-notch fa-spin mr-10\"></i>Sila tunggu..");
            $('#btn_submit').prop('disabled', true);
            var data = $("#form_pilkk").serialize();
            var action = $('#post_pengesahan_isu_lokasi_kanta_komuniti').val();
            var btn_text;
            if (action == 'edit') {
                url = pengesahan_isu_lokasi_kk_config.routes.pengesahan_isu_lokasi_kk_url;
                type = "POST";
            ...
            $.ajax({
                ...
                $('[name=pilkk_status]').removeClass("is-invalid");
                $('[name=pilkk_disahkan_note]').removeClass("is-invalid");
            
            if(response.errors){
                $.each(response.errors, function(index, error){
                    if(index == 'pilkk_status') {
                        $('[name=pilkk_status]').addClass("is-invalid");
                        $('.error_pilkk_status').html(error);
                    }

                    if(index == 'pilkk_disahkan_note') {
                        $('[name=pilkk_disahkan_note]').addClass("is-invalid");
                        $('.error_pilkk_disahkan_note').html(error);
                    }
                });
