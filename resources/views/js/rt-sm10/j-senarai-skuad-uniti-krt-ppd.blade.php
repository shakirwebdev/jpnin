...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_skuad_uniti_config = {
			routes: {
				senarai_skuad_unit_url: "/rt/sm10/senarai-skuad-uniti-krt-ppd"
			}
		};

        $("#ssukpd_krt_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#ssuk_skuad_uniti_id').find('option').remove();
			$('#ssuk_skuad_uniti_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					...
					success: function (data) {
						$('#ssuk_skuad_uniti_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#ssuk_skuad_uniti_id").on( 'change', function () {
	...
	function paparan_profile_skuad_uniti(id){
		window.location.href = "{{ route('rt-sm10.senarai_skuad_uniti_krt_ppd_1','') }}"+"/"+id;
	}
