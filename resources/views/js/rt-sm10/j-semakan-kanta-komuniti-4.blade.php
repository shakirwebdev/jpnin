...
<script type="text/javascript">    
	...
	$(document).ready( function () {
        ...
        /* Maklumat Profile Kanta Komuniti */
			...
			$('#skk_kanta_komuniti_id').val("{{$kanta_komuniti->id}}");

        /* Button */
            $('#btn_back').click(function(){
                window.location.href = "{{ route('rt-sm10.semakan_kanta_komuniti_3','') }}"+"/"+"{{$kanta_komuniti->id}}";
            });
    ...
    /* action submit */
        ...
        var semakan_kanta_komuniti_config = {
            routes: {
                semakan_kanta_komuniti_url: "{{ route('rt-sm10.post_semakan_kanta_komuniti') }}",
            }
        };
        ...
        $(document).on('submit', '#form_skk', function(event){    
            ...
            var action = $('#post_semakan_kanta_komuniti').val();
            var btn_text;
            if (action == 'edit') {
                url = semakan_kanta_komuniti_config.routes.semakan_kanta_komuniti_url;
                type = "POST";
            ...
            if(response.errors){
                ...
                $('#btn_submit').html(btn_text);
                    $('#btn_submit').prop('disabled', false); 
                    window.location.href = "{{route('rt-sm10.semakan_kanta_komuniti')}}";
                }
            });
        });
