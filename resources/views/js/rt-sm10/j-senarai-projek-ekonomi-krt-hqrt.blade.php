...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_projek_ekonomi_config = {
			routes: {
				senarai_projek_ekonomi_url: "/rt/sm10/senarai-projek-ekonomi-krt-hqrt"
			}
		};

        $("#spehq_state_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#spehq_daerah_id').find('option').remove();
			$('#spehq_daerah_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					url: senarai_projek_ekonomi_config.routes.senarai_projek_ekonomi_url,
					data: {type: 'get_daerah', value: value},
					success: function (data) {
						$('#spehq_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
        ...
        $("#spehq_daerah_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#spehq_krt_id').find('option').remove();
			$('#spehq_krt_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					url: senarai_projek_ekonomi_config.routes.senarai_projek_ekonomi_url,
					...
					success: function (data) {
						$('#spehq_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
						$.each(data,function(key, obj) 
						{
							$('#spehq_krt_id')
							.append($('<option>')
		...
		$("#spehq_krt_id").on( 'change', function () {
	...
	function paparan_projek_ekonomi(id){
		window.location.href = "{{ route('rt-sm10.senarai_projek_ekonomi_krt_hqrt_1','') }}"+"/"+id;
	}
