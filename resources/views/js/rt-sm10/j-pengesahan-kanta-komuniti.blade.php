...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var pengesahan_kanta_komuniti_config = {
			routes: {
				pengesahan_kanta_komuniti_url: "/rt/sm10/pengesahan-kanta-komuniti"
			}
		};
		...
		$("#pkkhq_daerah_id").on( 'change', function () {
		...
		var senarai_permohonan_kanta_komuniti = $('#senarai_permohonan_kanta_komuniti').DataTable( {
    		processing: true,
        	serverSide: true,
			ajax: {url: pengesahan_kanta_komuniti_config.routes.pengesahan_kanta_komuniti_url},
        	"language": {
	            "paginate": {
	                "previous": "Sebelumnya",
	                "next": "Seterusnya",
			...
			"aoColumnDefs":[{          
                ...
                "mRender": function ( value, type, full )  {
                    if (full.status == '6') {
                        button_a = '<button type="button" class="btn btn-icon" title="Pengesahan Permohonan Koperasi" onclick="pengesahan_permohonan_kanta_komuniti(\'' + full.id + '\');"><i class="fa fa-edit"></i></button>';
					    return button_a;
                    } else {
                        button_b = '';
                        return button_b;
	...
	function pengesahan_permohonan_kanta_komuniti(id){
		window.location.href = "{{ route('rt-sm10.pengesahan_kanta_komuniti_1','') }}"+"/"+id;
	}
