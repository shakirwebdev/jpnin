...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_sejiwa_config = {
			routes: {
				senarai_sejiwa_url: "/rt/sm10/senarai-sejiwa-krt-ppn"
			}
		};

        $("#sskpn_daerah_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#sskpn_krt_id').find('option').remove();
			$('#sskpn_krt_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					...
					success: function (data) {
						$('#sskpn_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
        ...
        $("#sskpn_krt_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
		...
		$("#sejiwa_nama").on( 'change', function () {
	...
	function paparan_sejiwa(id){
		window.location.href = "{{ route('rt-sm10.senarai_sejiwa_krt_ppn_1','') }}"+"/"+id;
	}
