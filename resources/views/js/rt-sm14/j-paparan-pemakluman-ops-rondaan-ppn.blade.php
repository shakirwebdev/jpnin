...
<script type="text/javascript">    
    ...
    $(document).ready( function () {
        ...
        var senarai_ops_rondaan_config = {
            routes: {
                senarai_ops_rondaan_config_url: "/rt/sm14/paparan-pemakluman-ops-rondaan-ppn"
            }
        };

        $("#pporpn_daerah_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#pporpn_krt_id').find('option').remove();
			$('#pporpn_krt_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					...
					success: function (data) {
						$('#pporpn_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#pporpn_krt_id").on( 'change', function () {
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
			$('#pporpn_srs_id').find('option').remove();
			$('#pporpn_srs_id').prop("disabled", false);
			if (selectedIndex !== '0') {
				$.ajax({
					type: "GET",
					url: senarai_ops_rondaan_config.routes.senarai_ops_rondaan_config_url,
					...
					success: function (data) {
						$('#pporpn_srs_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
						$.each(data,function(key, obj) 
						{
							$('#pporpn_srs_id')
							.append($('<option>')
		...
		$("#pporpn_srs_id").on( 'change', function () {
            senarai_pemakluman_ops_rondaan.search( $(this).val() ).draw();
        });
	...
	function view_pemakluman_ops_rondaan(id){
		window.location.href = "{{ route('rt-sm14.surat_pemakluman_operasi_rondaan_ppn','') }}"+"/"+id;
	}
