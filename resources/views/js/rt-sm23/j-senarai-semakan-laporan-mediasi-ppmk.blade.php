...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var senarai_semakan_laporan_mediasi_config = {
			routes: {
				senarai_semakan_laporan_mediasi_url: "/rt/sm23/senarai-semakan-laporan-mediasi-ppmk"
			}
		};  

        $("#sslmmk_daerah_id").on( 'change', function () {
    	...
    	var senarai_permohonan_laporan_mediasi = $('#senarai_permohonan_laporan_mediasi').DataTable( {
			...
			"aoColumnDefs":[{          
                ...
                "mRender": function ( value, type, full )  {
                    if (full.status == '3') {
                        button_a = '<button type="button" class="btn btn-icon" title="Semakan Permohonan Laporan Kes Mediasi" onclick="semakan_permohonan_laporan_mendiasi(\'' + full.id + '\');"><i class="fa fa-edit"></i></button>';
					    return button_a;
                    }else {
                        button_b = '';
                        return button_b;
	...
	function semakan_permohonan_laporan_mendiasi(id){
		window.location.href = "{{ route('rt-sm23.semakan_laporan_mediasi_mkp_ppmk','') }}"+"/"+id;
	}
