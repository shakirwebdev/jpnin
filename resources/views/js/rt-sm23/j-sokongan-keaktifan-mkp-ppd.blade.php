...
<script type="text/javascript">    
	...
	$(document).ready( function () {
        ...
        /* Maklumat MKP */
			$('#skmppd_mkp_nama').val("{{$mkp->mkp_nama}}");
            $('#skmppd_mkp_no_ic').val("{{$mkp->mkp_no_ic}}");
            $('#skmppd_mkp_no_phone').val("{{$mkp->mkp_no_phone}}");
			$('#skmppd_mkp_email').val("{{$mkp->user_email}}");
    ...
    /* click submit sokongan permohonan mkp */
		//my custom script
		var sokongan_mkp_keaktifan_ppd_config = {
			routes: {
				sokongan_mkp_keaktifan_ppd_url: "{{ route('rt-sm23.post_sokongan_mkp_keaktifan_ppd') }}",
			}
		};

        $(document).on('submit', '#form_skmppd', function(event){    
			event.preventDefault();
			$('#btn_submit').html("<i class=\"fa fa-circle-o-notch fa-spin mr-10\"></i>Sila tunggu..");
			$('#btn_submit').prop('disabled', true);
			var data = $("#form_skmppd").serialize();
			var action = $('#post_sokongan_mkp_keaktifan_ppd').val();
			var btn_text;
			if (action == 'edit') {
				url = sokongan_mkp_keaktifan_ppd_config.routes.sokongan_mkp_keaktifan_ppd_url;
				type = "POST";
			...
			$.ajax({
				...
				$('[name=skmppd_mkp_keaktifan_status]').removeClass("is-invalid");
				$('[name=skmppd_disokong_note]').removeClass("is-invalid");
				
				if(response.errors){
					$.each(response.errors, function(index, error){
						if(index == 'skmppd_mkp_keaktifan_status') {
							$('[name=skmppd_mkp_keaktifan_status]').addClass("is-invalid");
							$('.error_skmppd_mkp_keaktifan_status').html(error);
						}

						if(index == 'skmppd_disokong_note') {
							$('[name=skmppd_disokong_note]').addClass("is-invalid");
							$('.error_skmppd_disokong_note').html(error);
						}
