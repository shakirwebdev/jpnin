...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var senarai_laporan_kes_config = {
			routes: {
				senarai_laporan_kes_url: "/rt/sm23/senarai-laporan-kes-pp"
			}
		};  

        $("#slkp_state_id").on( 'change', function () {
			senarai_laporan_kes.column('1:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#slkp_daerah_id').find('option').remove();
            $('#slkp_daerah_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#slkp_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
        ...
        $("#slkp_daerah_id").on( 'change', function () {
	...
	function paparan_laporan_kes(id){
		window.location.href = "{{ route('rt-sm23.laporan_kes_mkp_pp','') }}"+"/"+id;
	}
