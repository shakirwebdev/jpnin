...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		/* Maklumat Kes Dalam Krt */
            ...
            $('#spmpd_state_id').val("{{$mkp->mkp_pemohon_state_id}}");
			$('#spmpd_daerah_id').val("{{$mkp->mkp_pemohon_daerah_id}}");
			$('#spmpd_krt_profile_id').val("{{$mkp->krt_profile_id}}");

		/* Maklumat Status Sokongan */
			$('#spmpd_disemak_note').on('keyup keypress', function(e) {
        ...
        /* Maklumat MKP */
            ...
            $('#spmpd_mkp_pemohon_nama').val("{{$mkp->mkp_pemohon_nama}}");
		...
		/* Button */
			$('#btn_back').click(function(){
				window.location.href = "{{ route('rt-sm23.senarai_sokongan_pelanjutan_mkp_ppd') }}";
			});
    });
    ...
    /* click submit sokongan pelanjutan mkp */
		//my custom script
		var sokongan_pelanjutan_mkp_ppd_config = {
			routes: {
				sokongan_pelanjutan_mkp_ppd_url: "{{ route('rt-sm23.post_sokongan_pelanjutan_mkp_ppd') }}",
			}
		};
        ...
        $(document).on('submit', '#form_spmpd', function(event){    
			...
			var action = $('#post_sokongan_pelanjutan_mkp_ppd').val();
			var btn_text;
			if (action == 'edit') {
				url = sokongan_pelanjutan_mkp_ppd_config.routes.sokongan_pelanjutan_mkp_ppd_url;
				type = "POST";
