...
<script type="text/javascript">    
	...
	var senarai_permohonan_srs_config = {
		routes: {
			senarai_permohonan_srs_url: "/rt/sm12/peraku-permohonan-penubuhan-srs"
		}
	};	
    
	$(document).ready( function () {

        $("#ppps_negeri_id").on( 'change', function () {
            var value = $(this).find('option:selected').val();
            var selectedIndex = $(this).find('option:selected').index();
            $('#ppps_daerah_id').find('option').remove();
            $('#ppps_daerah_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#ppps_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
    	...
    	var senarai_permohonan_penubuhan_srs = $('#senarai_permohonan_penubuhan_srs').DataTable( {
			...
			"aoColumnDefs":[{          
				...
				sClass: 'text-center',
                "mRender": function ( value, type, full )  {
                    button_a = '<button type="button" class="btn btn-icon" title="Semak Permohonan Penubuhan SRS" id="edit-jpnin" onclick="peraku_permohonan_srs(\'' + full.id + '\');"><i class="fa fa-edit"></i></button>';
					return button_a;
                }
            }],
            "order": [[ 0, 'asc' ]],
	...
	function peraku_permohonan_srs(id){
        window.location.href = "{{ route('rt-sm12.peraku_permohonan_penubuhan_srs_hq','') }}"+"/"+id;
    }
