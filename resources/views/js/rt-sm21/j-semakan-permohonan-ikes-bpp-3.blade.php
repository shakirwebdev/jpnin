...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		/* Maklumat Status Semakan */
			...
			$('#apipn3_disemak_note').on('keyup keypress', function(e) {
	...
	/* click submit akuan permohonan ikes */
		//my custom script
		var semakan_permohonan_ikes_config = {
			routes: {
				semakan_permohonan_ikes_url: "{{ route('rt-sm21.post_semakan_permohonan_ikes') }}",
			}
		};
		...
		$(document).on('submit', '#form_apipn3', function(event){    
			...
			var action = $('#post_semakan_permohonan_ikes').val();
			var btn_text;
			if (action == 'edit') {
				url = semakan_permohonan_ikes_config.routes.semakan_permohonan_ikes_url;
				type = "POST";
			...
			$.ajax({
				...
				$('[name=apipn3_disemak_note]').removeClass("is-invalid");
				...
				if(response.errors){
					$.each(response.errors, function(index, error){
						...
						if(index == 'apipn3_disemak_note') {
							$('[name=apipn3_disemak_note]').addClass("is-invalid");
							$('.error_apipn3_disemak_note').html(error);
						}
					...
					$('#btn_submit').html(btn_text);
					$('#btn_submit').prop('disabled', false); 
					window.location.href = "{{route('rt-sm21.senarai_semakan_permohonan_ikes_bpp')}}";
				}
			});
		});
