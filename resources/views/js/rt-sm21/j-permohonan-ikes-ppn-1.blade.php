...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		/* Maklumat Kes Dalam Krt */
			if("{{$ikes->hasRT}}" == 1){
				$('#pipn4_hasRT').attr("checked", "checked");
			}
			$('#pipn4_negeri_id').val("{{$ikes->krt_state_id}}");
			$('#pipn4_daerah_id').val("{{$ikes->krt_daerah_id}}");
			$('#pipn4_krt_profile_id').val("{{$ikes->krt_profile_id}}");

		/* Maklumat Pemohon */
			$('#pipn4_user_fullname').val("{{$ikes->nama_permohon}}");
			$('#pipn4_user_no_ic').val("{{$ikes->ic_pemohon}}");
			$('#pipn4_user_no_phone').val("{{$ikes->phone_pemohon}}");
			$('#pipn4_dihantar_alamat').val("{{$ikes->dihantar_alamat}}");
		
		/* Maklumat Kes Kejadian */
			$('#pipn4_ikes_bil_terlibat').val("{{$ikes->ikes_bil_terlibat}}");
			$('#pipn4_status_etnik_id').val("{{$ikes->status_etnik_id}}");
			$('#pipn4_status_warganegara_id').val("{{$ikes->status_warganegara_id}}");
			...
			$('#pipn4_ikes_bil_tangkapan').val("{{$ikes->ikes_bil_tangkapan}}");

			$('#pipn5_ikes_id').val("{{$ikes->id}}");

		/* Maklumat Note Kemaskini */
			$('#pipn4_status').val("{{$ikes->status}}");

			if($('#pipn4_status').val() == '11'){
				$("#pipn4_perlu_kemaskini").show();
				$('#pipn4_status_description').html("{{$ikes->status_description}}");
				$('#pipn4_disemak_note').html("{{$ikes->disemak_note}}");
			}

			if($('#pipn4_status').val() == '12'){
				$("#pipn4_perlu_kemaskini").show();
				$('#pipn4_status_description').html("{{$ikes->status_description}}");
				$('#pipn4_disahkan_note').html("{{$ikes->disahkan_note}}");
			}

		/* Button */
        	$('#btn_back').click(function(){
				window.location.href = "{{route('rt-sm21.permohonan_ikes_ppn','')}}"+"/"+"{{$ikes->id}}";
			});
	...
	/* click btn next */
		//my custom script
		var permohonan_ikes_ppn_2_config = {
			routes: {
				permohonan_ikes_ppn_2_url: "{{ route('rt-sm21.post_permohonan_ikes_ppn_2') }}",
			}
		};

		$(document).on('submit', '#form_pipn5', function(event){    
			event.preventDefault();
			$('#btn_next').html("<i class=\"fa fa-circle-o-notch fa-spin mr-10\"></i>Sila tunggu..");
			$('#btn_next').prop('disabled', true);
			var data   = $("#form_pipn4, #form_pipn5").serialize();
			var action = $('#post_permohonan_ikes_ppn_2').val();
			var btn_text;
			if (action == 'edit') {
				url = permohonan_ikes_ppn_2_config.routes.permohonan_ikes_ppn_2_url;
				type = "POST";
			...
			$.ajax({
				...
				$('[name=pipn4_ikes_bil_terlibat]').removeClass("is-invalid");
				$('[name=pipn4_status_etnik_id]').removeClass("is-invalid");
				$('[name=pipn4_status_warganegara_id]').removeClass("is-invalid");
				$('[name=pipn4_ikes_bil_tangkapan]').removeClass("is-invalid");
				
				if(response.errors){
					$.each(response.errors, function(index, error){
						if(index == 'pipn4_ikes_bil_terlibat') {
							$('[name=pipn4_ikes_bil_terlibat]').addClass("is-invalid");
							$('.error_pipn4_ikes_bil_terlibat').html(error);
						}

						if(index == 'pipn4_status_etnik_id') {
							$('[name=pipn4_status_etnik_id]').addClass("is-invalid");
							$('.error_pipn4_status_etnik_id').html(error);
						}

						if(index == 'pipn4_status_warganegara_id') {
							$('[name=pipn4_status_warganegara_id]').addClass("is-invalid");
							$('.error_pipn4_status_warganegara_id').html(error);
						}

						if(index == 'pipn4_ikes_bil_tangkapan') {
							$('[name=pipn4_ikes_bil_tangkapan]').addClass("is-invalid");
							$('.error_pipn4_ikes_bil_tangkapan').html(error);
						}
