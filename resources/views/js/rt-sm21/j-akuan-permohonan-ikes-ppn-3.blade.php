...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		/* Maklumat Status Memperakui */
			...
			$('#apipn3_diakui_note').on('keyup keypress', function(e) {
	...
	/* click submit akuan permohonan ikes */
		//my custom script
		var akui_permohonan_ikes_config = {
			routes: {
				akui_permohonan_ikes_url: "{{ route('rt-sm21.post_akui_permohonan_ikes') }}",
			}
		};
		...
		$(document).on('submit', '#form_apipn3', function(event){    
			...
			var action = $('#post_akui_permohonan_ikes').val();
			var btn_text;
			if (action == 'edit') {
				url = akui_permohonan_ikes_config.routes.akui_permohonan_ikes_url;
				type = "POST";
			...
			$.ajax({
				...
				$('[name=apipn3_diakui_note]').removeClass("is-invalid");
				...
				if(response.errors){
					$.each(response.errors, function(index, error){
						...
						if(index == 'apipn3_diakui_note') {
							$('[name=apipn3_diakui_note]').addClass("is-invalid");
							$('.error_apipn3_diakui_note').html(error);
						}
