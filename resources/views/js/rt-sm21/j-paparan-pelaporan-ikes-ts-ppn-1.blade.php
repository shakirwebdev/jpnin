...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		/* Maklumat Kes Dalam Krt */
			if("{{$ikes->hasRT}}" == 1){
				$('#ppip1_hasRT').attr("checked", "checked");
			}
			$('#ppip1_negeri_id').val("{{$ikes->krt_state_id}}");
			$('#ppip1_daerah_id').val("{{$ikes->krt_daerah_id}}");
			$('#ppip1_krt_profile_id').val("{{$ikes->krt_profile_id}}");

		/* Maklumat Pemohon */
			$('#ppip1_user_fullname').val("{{$ikes->nama_permohon}}");
			$('#ppip1_user_no_ic').val("{{$ikes->ic_pemohon}}");
			$('#ppip1_user_no_phone').val("{{$ikes->phone_pemohon}}");
			$('#ppip1_dihantar_alamat').val("{{$ikes->dihantar_alamat}}");

		/* Maklumat Kes Kejadian */
			$('#ppip1_ikes_bil_terlibat').val("{{$ikes->ikes_bil_terlibat}}");
			$('#ppip1_status_etnik_id').val("{{$ikes->status_etnik_id}}");
			$('#ppip1_status_warganegara_id').val("{{$ikes->status_warganegara_id}}");
			$('#ppip1_ikes_bil_tangkapan').val("{{$ikes->ikes_bil_tangkapan}}");
		...
		/* Button */
			$('#btn_back').click(function(){
				window.location.href = "{{route('rt-sm21.paparan_pelaporan_ikes_ts_ppn','')}}"+"/"+"{{$ikes->id}}";
			});

			$('#btn_next').click(function(){
				window.location.href = "{{route('rt-sm21.paparan_pelaporan_ikes_ts_ppn_2','')}}"+"/"+"{{$ikes->id}}";
			});
