...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		/* Maklumat Kes Dalam Krt */
			if("{{$ikes->hasRT}}" == 1){
				$('#pi4_hasRT').attr("checked", "checked");
			}
			$('#pi4_negeri_id').val("{{$ikes->krt_state_id}}");
			$('#pi4_daerah_id').val("{{$ikes->krt_daerah_id}}");
			$('#pi4_krt_profile_id').val("{{$ikes->krt_profile_id}}");

		/* Maklumat Pemohon */
			$('#pi4_user_fullname').val("{{$ikes->nama_permohon}}");
			$('#pi4_user_no_ic').val("{{$ikes->ic_pemohon}}");
			$('#pi4_user_no_phone').val("{{$ikes->phone_pemohon}}");
			$('#pi4_dihantar_alamat').val("{{$ikes->dihantar_alamat}}");
		
		/* Maklumat Kes Kejadian */
			$('#pi4_ikes_bil_terlibat').val("{{$ikes->ikes_bil_terlibat}}");
			$('#pi4_status_etnik_id').val("{{$ikes->status_etnik_id}}");
			$('#pi4_status_warganegara_id').val("{{$ikes->status_warganegara_id}}");
			...
			$('#pi4_ikes_bil_tangkapan').val("{{$ikes->ikes_bil_tangkapan}}");

			$('#pi5_ikes_id').val("{{$ikes->id}}");

		/* Maklumat Note Kemaskini */
			$('#pi4_status').val("{{$ikes->status}}");
				
			if($('#pi4_status').val() == '5'){
				$("#pi4_perlu_kemaskini").show();
				$('#pi4_status_description').html("{{$ikes->status_description}}");
				$('#pi4_diakui_note').html("{{$ikes->diakui_note}}");
			}

			if($('#pi4_status').val() == '7'){
				$("#pi4_perlu_kemaskini").show();
				$('#pi4_status_description').html("{{$ikes->status_description}}");
				$('#pi4_disemak_note').html("{{$ikes->disemak_note}}");
			}

			if($('#pi4_status').val() == '8'){
				$("#pi4_perlu_kemaskini").show();
				$('#pi4_status_description').html("{{$ikes->status_description}}");
				$('#pi4_disahkan_note').html("{{$ikes->disahkan_note}}");
			}

		/* Button */
        	$('#btn_back').click(function(){
				window.location.href = "{{route('rt-sm21.permohonan_ikes','')}}"+"/"+"{{$ikes->id}}";
			});
	...
	/* click btn next */
		//my custom script
		var permohonan_ikes_2_config = {
			routes: {
				permohonan_ikes_2_url: "{{ route('rt-sm21.post_permohonan_ikes_2') }}",
			}
		};

		$(document).on('submit', '#form_pi5', function(event){    
			event.preventDefault();
			$('#btn_next').html("<i class=\"fa fa-circle-o-notch fa-spin mr-10\"></i>Sila tunggu..");
			$('#btn_next').prop('disabled', true);
			var data   = $("#form_pi4, #form_pi5").serialize();
			var action = $('#post_permohonan_ikes_2').val();
			var btn_text;
			if (action == 'edit') {
				url = permohonan_ikes_2_config.routes.permohonan_ikes_2_url;
				type = "POST";
			...
			$.ajax({
				...
				$('[name=pi4_ikes_bil_terlibat]').removeClass("is-invalid");
				$('[name=pi4_status_etnik_id]').removeClass("is-invalid");
				$('[name=pi4_status_warganegara_id]').removeClass("is-invalid");
				$('[name=pi4_ikes_bil_tangkapan]').removeClass("is-invalid");
				
				if(response.errors){
					$.each(response.errors, function(index, error){
						if(index == 'pi4_ikes_bil_terlibat') {
							$('[name=pi4_ikes_bil_terlibat]').addClass("is-invalid");
							$('.error_pi4_ikes_bil_terlibat').html(error);
						}

						if(index == 'pi4_status_etnik_id') {
							$('[name=pi4_status_etnik_id]').addClass("is-invalid");
							$('.error_pi4_status_etnik_id').html(error);
						}

						if(index == 'pi4_status_warganegara_id') {
							$('[name=pi4_status_warganegara_id]').addClass("is-invalid");
							$('.error_pi4_status_warganegara_id').html(error);
						}

						if(index == 'pi4_ikes_bil_tangkapan') {
							$('[name=pi4_ikes_bil_tangkapan]').addClass("is-invalid");
							$('.error_pi4_ikes_bil_tangkapan').html(error);
						}
