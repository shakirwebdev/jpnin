...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		/* Maklumat Kes Dalam Krt */
			if("{{$ikes->hasRT}}" == 1){
				$('#pi8_hasRT').attr("checked", "checked");
			}
			$('#pi8_negeri_id').val("{{$ikes->krt_state_id}}");
			$('#pi8_daerah_id').val("{{$ikes->krt_daerah_id}}");
			$('#pi8_krt_profile_id').val("{{$ikes->krt_profile_id}}");

		/* Maklumat Pemohon */
			$('#pi8_user_fullname').val("{{$ikes->nama_permohon}}");
			$('#pi8_user_no_ic').val("{{$ikes->ic_pemohon}}");
			$('#pi8_user_no_phone').val("{{$ikes->phone_pemohon}}");
			$('#pi8_dihantar_alamat').val("{{$ikes->dihantar_alamat}}");

		/* Maklumat Kes Kejadian */
			$('#pi8_ikes_keadaan_semasa').val("{{$ikes->ikes_keadaan_semasa}}");
			$('#pi8_ikes_jangkaan_keadaan').html("{{$ikes->ikes_jangkaan_keadaan}}");
			...
			$('#pi8_ikes_jangkaan_keadaan').summernote({
				height: 200
			});
			

			$('#pi9_spk_ikes_id').val("{{$ikes->id}}");
        	$('#pi10_ikes_id').val("{{$ikes->id}}");

		/* Maklumat Note Kemaskini */
			$('#pi8_status').val("{{$ikes->status}}");
				
			if($('#pi8_status').val() == '5'){
				$("#pi8_perlu_kemaskini").show();
				$('#pi8_status_description').html("{{$ikes->status_description}}");
				$('#pi8_diakui_note').html("{{$ikes->diakui_note}}");
			}

			if($('#pi8_status').val() == '7'){
				$("#pi8_perlu_kemaskini").show();
				$('#pi8_status_description').html("{{$ikes->status_description}}");
				$('#pi8_disemak_note').html("{{$ikes->disemak_note}}");
			}

			if($('#pi8_status').val() == '8'){
				$("#pi8_perlu_kemaskini").show();
				$('#pi8_status_description').html("{{$ikes->status_description}}");
				$('#pi8_disahkan_note').html("{{$ikes->disahkan_note}}");
			}
	...
	/* click add dokument */
		$(document).on('submit', '#form_pi9', function(event){
			var info = $('.error_form_pi9');
			event.preventDefault();

			var form_data = new FormData();
			form_data.append("pi9_file_title", $("#pi9_file_title").val() );
			form_data.append("pi9_file_catatan", $("#pi9_file_catatan").val() );
			form_data.append("pi9_file_dokument",  $("#pi9_file_dokument")[0].files[0]);
			form_data.append("pi9_spk_ikes_id", $("#pi9_spk_ikes_id").val() );
			form_data.append("add_spk_ikes_dokument", "add" );
			console.log(form_data);
			$('#btn_save_dokument').html("<i class=\"fa fa-circle-o-notch fa-spin mr-10\"></i>Sila tunggu..");
			$('#btn_save_dokument').prop('disabled', true);
			btn_text = '<i class="fe fe-plus mr-2"></i>Tambah';
			url = "{{ route('rt-sm21.add_spk_ikes_dokument') }}";
			type = "POST";
			$.ajaxSetup({
				headers: {
	...
	/* click btn back */
		//my custom script
		var permohonan_ikes_4_back_config = {
			routes: {
				permohonan_ikes_4_back_url: "{{ route('rt-sm21.post_permohonan_ikes_4_back') }}",
			}
		};
		...
		$(document).on('click', '#btn_back', function(event){    
			...
			var data   = $("#form_pi8, #form_pi10").serialize();
			var action = $('#post_permohonan_ikes_4').val();
			var btn_text;
			if (action == 'edit') {
				url = permohonan_ikes_4_back_config.routes.permohonan_ikes_4_back_url;
				type = "POST";
			...
			$.ajax({
				...
				$('[name=pi8_ikes_keadaan_semasa]').removeClass("is-invalid");
				$('[name=pi8_ikes_jangkaan_keadaan]').removeClass("is-invalid");
				
				if(response.errors){
					$.each(response.errors, function(index, error){
						if(index == 'pi8_ikes_keadaan_semasa') {
							$('[name=pi8_ikes_keadaan_semasa]').addClass("is-invalid");
							$('.error_pi8_ikes_keadaan_semasa').html(error);
						}

						if(index == 'pi8_ikes_jangkaan_keadaan') {
							$('[name=pi8_ikes_jangkaan_keadaan]').addClass("is-invalid");
							$('.error_pi8_ikes_jangkaan_keadaan').html(error);
						}
					...
					$('#btn_submit').html(btn_text);
					$('#btn_submit').prop('disabled', false); 
					window.location.href = "{{route('rt-sm21.permohonan_ikes_2','')}}"+"/"+"{{$ikes->id}}";
				}
			});
		});
	...
	/* click btn Submit */
		//my custom script
		var permohonan_ikes_4_config = {
			routes: {
				permohonan_ikes_4_url: "{{ route('rt-sm21.post_permohonan_ikes_4') }}",
			}
		};

		$(document).on('submit', '#form_pi10', function(event){    
			event.preventDefault();
			...
			var btn_text;
			if (action == 'edit') {
				url = permohonan_ikes_4_config.routes.permohonan_ikes_4_url;
				type = "POST";
