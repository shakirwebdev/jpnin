...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var new_daerah = $('#load_daerah_id').val();
		$("#kkhq_daerah_id").val(new_daerah);

		//my custom script
		var senarai_keaktifan_krt_config = {
			routes: {
				senarai_keaktifan_krt_url: "/rt/sm11/keaktifan-krt-ppd"
			}
		};
		...
		$.fn.myfunction = function() {
        	...
        	senarai_keaktifan_krt = $('#senarai_keaktifan_krt').DataTable( {
				...
				"aoColumnDefs":[{          
					...
					sClass: 'text-center',
					"mRender": function ( value, type, full )  {
						$v_markah_profile = (parseInt(full.markah_latar) + parseInt(full.markah_penduduk) + parseInt(full.markah_pekerjaan) + parseInt(full.markah_rumah) +  parseInt(full.markah_pertubuhan) + parseInt(full.markah_kemudahan) + parseInt(full.markah_sosial) + parseInt(full.markah_tempat_krt) + parseInt(full.markah_profil_peta)); 
					...
					"mRender": function ( value, type, full )  {
						if(full.markah_aktiviti == null)
							...
							return parseFloat(full.markah_aktiviti).toFixed(1)+'%';
					}
				},{          
					"aTargets": [ 8 ], 
					sClass: 'text-center',
					"mRender": function ( value, type, full )  {
						if(full.markah_mesyuarat == null)
							...
							return parseFloat(full.markah_mesyuarat).toFixed(1)+'%';
					}
				},{          
					"aTargets": [ 9 ],  
					sClass: 'text-center',
					"mRender": function ( value, type, full )  {
						if(full.markah_kewangan == null)
							...
							return parseFloat(full.markah_kewangan).toFixed(1)+'%';
					}
				},{          
					"aTargets": [ 10 ], 
					sClass: 'text-center',
					"mRender": function ( value, type, full )  {
						if(full.keaktifan_markah == null)
							...
							return parseFloat(full.keaktifan_markah).toFixed(1)+'%';
					}
				},{          
					"aTargets": [ 11 ],  
					sClass: 'text-center',
					...
					"mRender": function ( value, type, full )  {
						if (full.status == 'Belum Selesai') {
							button_a = '<button type="button" class="btn btn-icon" title="Kemaskini Permakahan KRT" onclick="load_add_markah_keaktifan_krt(\'' + full.id + '\',\'' + full.keaktifan_markah + '\');"><i class="fa fa-edit"></i></button>';
							return button_a;
						} else {
							button_b = '';
							return button_b;
		...
		$("#kkhq_krt_id").on( 'change', function () {
            //senarai_keaktifan_krt.search( $(this).val() ).draw();
			$.fn.myfunction();
			//senarai_keaktifan_krt.search( 'FASA 1L BANDAR TASEK KESUMA' ).draw();
        }); 
