...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		var senarai_ajk_krt_ppd_config = {
			routes: {
				senarai_ajk_krt_ppd_url: "/rt/sm4/kad-keahlian-hqrt"
			}
		};
        ...
        $("#kkhq_state_id").on( 'change', function () {
            ...
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: senarai_ajk_krt_ppd_config.routes.senarai_ajk_krt_ppd_url,
                    data: {type: 'get_daerah', value: value},
                    success: function (data) {
                        $('#kkhq_daerah_id').append($('<option>').text('- Pilih daerah').attr('value', ''));
