...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_lanjut_mkp_config = {
			routes: {
				laporan_lanjut_mkp_url: "/rt/sm32/laporan-pelanjutan-mkp-ppmk"
			}
		}; 

        $("#lpmpn_daerah_id").on( 'change', function () {
			laporan_lanjut_mk.column('2:visible').search( $(this).val() ).draw();
		});

		$("#lpmpn_parlimen_id").on( 'change', function () {
			laporan_lanjut_mk.column('3:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lpmpn_dun_id').find('option').remove();
            $('#lpmpn_dun_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lpmpn_dun_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#lpmpn_dun_id").on( 'change', function () {
