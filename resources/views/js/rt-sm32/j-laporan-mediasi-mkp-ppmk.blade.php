...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_mediasi_mkp_config = {
			routes: {
				laporan_mediasi_mkp_url: "/rt/sm32/laporan-mediasi-mkp-ppmk"
			}
		}; 

        $("#lmpn_daerah_id").on( 'change', function () {
			laporan_mediasi_mk.column('2:visible').search( $(this).val() ).draw();
		});

		$("#lmpn_parlimen_id").on( 'change', function () {
			laporan_mediasi_mk.column('3:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lmpn_dun_id').find('option').remove();
            $('#lmpn_dun_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lmpn_dun_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#lmpn_dun_id").on( 'change', function () {
