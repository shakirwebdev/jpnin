...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_keaktifan_mk_config = {
			routes: {
				laporan_keaktifan_mk_url: "/rt/sm32/laporan-keaktifan-mk-ppd"
			}
		}; 
        ...
        var laporan_keaktifan_mk = $('#laporan_keaktifan_mk').DataTable( {
			...
			"aoColumnDefs":[{          
				...
				sClass: 'text-center',
                "mRender": function ( value, type, full )  {
                    button_a = '<button type="button" class="btn btn-icon" title="Paparan Keaktifan Mediator Komuniti Individu" onclick="paparan_keaktifan_mkp_ppd(\'' + full.id + '\');"><i class="fa fa-search"></i></button>';
					return button_a
                }
            }],
            "order": [[ 0, 'asc' ]],
    ...
    function paparan_keaktifan_mkp_ppd(id){
		window.location.href = "{{ route('rt-sm32.keaktifan_mkp_individu_ppd','') }}"+"/"+id;
	}
