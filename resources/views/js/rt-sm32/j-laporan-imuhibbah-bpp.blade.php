...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_imuhibbah_config = {
			routes: {
				laporan_imuhibbah_url: "/rt/sm32/laporan-imuhibbah-bpp"
			}
		};   

		$("#libp_state_id").on( 'change', function () {
			table_laporan_imuhibbah.column('1:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#libp_daerah_id').find('option').remove();
            $('#libp_daerah_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: laporan_imuhibbah_config.routes.laporan_imuhibbah_url,
                    data: {type: 'get_daerah', value: value},
                    success: function (data) {
                        $('#libp_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#libp_daerah_id").on( 'change', function () {
