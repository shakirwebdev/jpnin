...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_ikes_isu_semasa_config = {
			routes: {
				laporan_ikes_isu_semasa_url: "/rt/sm32/laporan-ikes-isu-semasa-ppmk"
			}
		}; 

		$("#lispn_daerah_id").on( 'change', function () {
			laporan_isu_semasa_ikes.column('2:visible').search( $(this).val() ).draw();
		});

		$("#lispn_parlimen_id").on( 'change', function () {
			laporan_isu_semasa_ikes.column('3:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lispn_dun_id').find('option').remove();
            $('#lispn_dun_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lispn_dun_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#lispn_dun_id").on( 'change', function () {
