...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_ikes_config = {
			routes: {
				laporan_ikes_url: "/rt/sm32/laporan-ikes-bpp"
			}
		}; 
        
    	$("#lib_state_id").on( 'change', function () {
			table_laporan_ikes.column('4:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lib_daerah_id').find('option').remove();
            $('#lib_daerah_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lib_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#lib_state_id").on( 'change', function () {
			...
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lib_parlimen_id').find('option').remove();
            $('#lib_parlimen_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
					url: laporan_ikes_config.routes.laporan_ikes_url,
                    ...
                    success: function (data) {
                        $('#lib_parlimen_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#lib_parlimen_id')
                            .append($('<option>')
		...
		$("#lib_daerah_id").on( 'change', function () {
			table_laporan_ikes.column('5:visible').search( $(this).val() ).draw();
		});

		$("#lib_parlimen_id").on( 'change', function () {
			table_laporan_ikes.column('7:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lib_dun_id').find('option').remove();
            $('#lib_dun_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: laporan_ikes_config.routes.laporan_ikes_url,
                    ...
                    success: function (data) {
                        $('#lib_dun_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#lib_dun_id')
                            .append($('<option>')
		...
		$("#lib_dun_id").on( 'change', function () {
            table_laporan_ikes.column('6:visible').search( $(this).val() ).draw();
		});

        $("#min_date").on( 'change', function () {
            table_laporan_ikes.column('1:visible').search( $(this).val() >= $(this).val() ).draw();
		});
