...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_ikes_isu_semasa_config = {
			routes: {
				laporan_ikes_isu_semasa_url: "/rt/sm32/laporan-ikes-isu-semasa-bpp"
			}
		}; 

		$("#lisb_state_id").on( 'change', function () {
			laporan_isu_semasa_ikes.column('1:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lisb_daerah_id').find('option').remove();
            $('#lisb_daerah_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lisb_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#lisb_daerah_id").on( 'change', function () {
		...
		$("#lisb_state_id").on( 'change', function () {
			...
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lisb_parlimen_id').find('option').remove();
            $('#lisb_parlimen_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
					url: laporan_ikes_isu_semasa_config.routes.laporan_ikes_isu_semasa_url,
                    ...
                    success: function (data) {
                        $('#lisb_parlimen_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#lisb_parlimen_id')
                            .append($('<option>')
		...
		$("#lisb_parlimen_id").on( 'change', function () {
			laporan_isu_semasa_ikes.column('3:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lisb_dun_id').find('option').remove();
            $('#lisb_dun_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: laporan_ikes_isu_semasa_config.routes.laporan_ikes_isu_semasa_url,
                    ...
                    success: function (data) {
                        $('#lisb_dun_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#lisb_dun_id')
                            .append($('<option>')
		...
		$("#lisb_dun_id").on( 'change', function () {
