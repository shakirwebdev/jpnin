...
<script type="text/javascript">    
	...
	$(document).ready( function () {
		...
		/* Maklumat KRT Yang Dimohon */
			...
			$('#kpk_krt_alamat').text("{{$profile_krt->krt_alamat}}");
		...
		$('#btn_back').click(function(){
			window.location.href = "{{route('rt-sm2.profile_krt_ppd_1','')}}"+"/"+"{{$profile_krt->id}}";
		});

		$('#btn_next').click(function(){
			window.location.href = "{{route('rt-sm2.profile_krt_ppd_3','')}}"+"/"+"{{$profile_krt->id}}";
		});
