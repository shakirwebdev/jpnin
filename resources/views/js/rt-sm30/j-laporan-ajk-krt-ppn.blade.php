...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_ajk_krt_config = {
			routes: {
				laporan_ajk_krt_url: "/rt/sm30/laporan-ajk-krt-ppn"
			}
		}; 

        $("#lakppn_parlimen_id").on( 'change', function () {
			laporan_ajk_krt.search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lakppn_dun_id').find('option').remove();
            $('#lakppn_dun_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lakppn_dun_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
        ...
        $("#lakppn_dun_id").on( 'change', function () {
			laporan_ajk_krt.search( $(this).val() ).draw();
		});

        $("#lakppn_daerah_id").on( 'change', function () {
			laporan_ajk_krt.search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lakppn_krt_id').find('option').remove();
            $('#lakppn_krt_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: laporan_ajk_krt_config.routes.laporan_ajk_krt_url,
                    ...
                    success: function (data) {
                        $('#lakppn_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#lakppn_krt_id')
                            .append($('<option>')
        ...
        $("#lakppn_krt_id").on( 'change', function () {
			laporan_ajk_krt.search( $(this).val() ).draw();
		});

        $("#lakppn_kaum_id").on( 'change', function () {
			laporan_ajk_krt.search( $(this).val() ).draw();
		});

        $("#lakppn_pendidikan_id").on( 'change', function () {
			laporan_ajk_krt.search( $(this).val() ).draw();
		});

        $("#lakppn_jawatan_id").on( 'change', function () {
			laporan_ajk_krt.search( $(this).val() ).draw();
		});
