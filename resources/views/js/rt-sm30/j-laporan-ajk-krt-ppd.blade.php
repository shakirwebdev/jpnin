...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_ajk_krt_config = {
			routes: {
				laporan_ajk_krt_url: "/rt/sm30/laporan-ajk-krt-ppd"
			}
		}; 

        $("#lakppd_parlimen_id").on( 'change', function () {
			laporan_ajk_krt.search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lakppd_dun_id').find('option').remove();
            $('#lakppd_dun_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lakppd_dun_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
        ...
        $("#lakppd_dun_id").on( 'change', function () {
			laporan_ajk_krt.search( $(this).val() ).draw();
		});

        $("#lakppd_krt_id").on( 'change', function () {
			laporan_ajk_krt.search( $(this).val() ).draw();
		});
