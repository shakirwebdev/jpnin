...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_maklumat_asas_config = {
			routes: {
				laporan_maklumat_asas_url: "/rt/sm30/laporan-maklumat-asas-krt-ppn"
			}
		}; 

		$("#lmakpn_parlimen_id").on( 'change', function () {
			laporan_maklumat_asas_rt.search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lmakpn_dun_id').find('option').remove();
            $('#lmakpn_dun_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lmakpn_dun_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#lmakpn_daerah_id").on( 'change', function () {
			laporan_maklumat_asas_rt.search( $(this).val() ).draw();
		});

		$("#lmakpn_pbt_id").on( 'change', function () {
			laporan_maklumat_asas_rt.search( $(this).val() ).draw();
		});

		$("#lmakpn_dun_id").on( 'change', function () {
			laporan_maklumat_asas_rt.search( $(this).val() ).draw();
		});
