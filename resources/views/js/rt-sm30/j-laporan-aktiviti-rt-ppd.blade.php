...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_aktiviti_config = {
			routes: {
				laporan_aktiviti_url: "/rt/sm30/laporan-aktiviti-rt-ppd"
			}
		}; 

        $("#larpd_agenda_id").on( 'change', function () {
			laporan_aktiviti.column('6:visible').search( $(this).val() ).draw();
		});

		$("#larpd_bidang_id").on( 'change', function () {
			laporan_aktiviti.column('7:visible').search( $(this).val() ).draw();
		});

		$("#larpd_k_aktiviti_id").on( 'change', function () {
			laporan_aktiviti.column('8:visible').search( $(this).val() ).draw();
		});

		$("#larpd_j_aktiviti_id").on( 'change', function () {
