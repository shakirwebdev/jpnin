...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_aktiviti_2_config = {
			routes: {
				laporan_aktiviti_2_url: "/rt/sm30/laporan-aktiviti-rt-2-hqrt"
			}
		}; 

        $("#larthq_state_id").on( 'change', function () {
			laporan_aktiviti_2.column('0:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#larthq_daerah_id').find('option').remove();
            $('#larthq_daerah_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: laporan_aktiviti_2_config.routes.laporan_aktiviti_2_url,
                    data: {type: 'get_daerah', value: value},
                    success: function (data) {
                        $('#larthq_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#larthq_daerah_id").on( 'change', function () {
