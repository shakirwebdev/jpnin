...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_rondaan_srs_config = {
			routes: {
				laporan_rondaan_srs_url: "{{ route('rt-sm31.laporan_rondaan_srs_hqsrs') }}",
            }
		}; 
        ...
        $("#lksrs_state_id").on( 'change', function () {
			senarai_rondaan_srs_table.column('0:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lksrs_daerah_id').find('option').remove();
            $('#lksrs_daerah_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lksrs_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
        ...
        $("#lksrs_daerah_id").on( 'change', function () {
			senarai_rondaan_srs_table.column('1:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lksrs_krt_id').find('option').remove();
            $('#lksrs_krt_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: laporan_rondaan_srs_config.routes.laporan_rondaan_srs_url,
                    ...
                    success: function (data) {
                        $('#lksrs_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#lksrs_krt_id')
                            .append($('<option>')
        ...
        $("#lksrs_krt_id").on( 'change', function () {
			senarai_rondaan_srs_table.column('2:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lksrs_srs_id').find('option').remove();
            $('#lksrs_srs_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: laporan_rondaan_srs_config.routes.laporan_rondaan_srs_url,
                    ...
                    success: function (data) {
                        $('#lksrs_srs_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#lksrs_srs_id')
                            .append($('<option>')
		...
		$("#lksrs_srs_id").on( 'change', function () {
