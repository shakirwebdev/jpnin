...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_pengendalian_kes_config = {
			routes: {
				laporan_pengendalian_kes_url: "/rt/sm31/laporan-pengendalian-kes-srs-ppn"
			}
		};  
		...
		$("#lpksrspn_daerah_id").on( 'change', function () {
			pengendalian_kes_srs_table.column('1:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lpksrspn_krt_id').find('option').remove();
            $('#lpksrspn_krt_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lpksrspn_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
