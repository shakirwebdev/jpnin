...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_srs_config = {
			routes: {
				laporan_srs_url: "/rt/sm31/laporan-srs-ppn"
			}
		};  

        $("#lsppn_daerah_id").on( 'change', function () {
			senarai_srs_table.search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lsppn_krt_id').find('option').remove();
            $('#lsppn_krt_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lsppn_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#lsppn_parlimen_id").on( 'change', function () {
			senarai_srs_table.search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lsppn_dun_id').find('option').remove();
            $('#lsppn_dun_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: laporan_srs_config.routes.laporan_srs_url,
                    ...
                    success: function (data) {
                        $('#lsppn_dun_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#lsppn_dun_id')
                            .append($('<option>')
		...
		$("#lsppn_dun_id").on( 'change', function () {
			senarai_srs_table.search( $(this).val() ).draw();
		});

		$("#lsppn_krt_id").on( 'change', function () {
			senarai_srs_table.search( $(this).val() ).draw();
		});
