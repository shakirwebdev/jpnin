...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_srs_config = {
			routes: {
				laporan_srs_url: "/rt/sm31/laporan-pembantalan-srs-hqsrs"
			}
		};  

		$("#lpshq_state_id").on( 'change', function () {
			senarai_pembatalan_srs_table.column('1:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lpshq_daerah_id').find('option').remove();
            $('#lpshq_daerah_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lpshq_daerah_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
		...
		$("#lpshq_daerah_id").on( 'change', function () {
			senarai_pembatalan_srs_table.column('2:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lpshq_krt_id').find('option').remove();
            $('#lpshq_krt_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: laporan_srs_config.routes.laporan_srs_url,
                    ...
                    success: function (data) {
                        $('#lpshq_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
                        $.each(data,function(key, obj) 
                        {
                            $('#lpshq_krt_id')
                            .append($('<option>')
		...
		$("#lpshq_krt_id").on( 'change', function () {
