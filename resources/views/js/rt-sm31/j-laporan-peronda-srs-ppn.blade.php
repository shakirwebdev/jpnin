...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		var laporan_peronda_srs_config = {
			routes: {
				laporan_peronda_srs_url: "/rt/sm31/laporan-peronda-srs-ppn"
			}
		};  

		$("#lppn_daerah_id").on( 'change', function () {
			senarai_ahli_peronda_table.column('2:visible').search( $(this).val() ).draw();
			var value = $(this).find('option:selected').val();
			var selectedIndex = $(this).find('option:selected').index();
            $('#lppn_krt_id').find('option').remove();
            $('#lppn_krt_id').prop("disabled", false);
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    ...
                    success: function (data) {
                        $('#lppn_krt_id').append($('<option>').text('-- Sila Pilih --').attr('value', ''));
