@section('page-script')
@include('js.modal.j-modal-view-ts-imuhibbah')
...
<script type="text/javascript"> 
	...
	$(document).ready( function () {
		...
		//my custom script
		var senarai_ts_imuhibbah_p_config = {
			routes: {
				senarai_ts_imuhibbah_p_url: "/rt/sm22/senarai-ts-imuhibbah-p"
			}
		};   
        ...
        $("#stip_state_id").on( 'change', function () {
            ...
            if (selectedIndex !== '0') {
                $.ajax({
                    type: "GET",
                    url: senarai_ts_imuhibbah_p_config.routes.senarai_ts_imuhibbah_p_url,
    	...
    	var senarai_at_imuhibbah = $('#senarai_at_imuhibbah').DataTable( {
			...
			ajax: {url: senarai_ts_imuhibbah_p_config.routes.senarai_ts_imuhibbah_p_url},
        	"language": {
	            "paginate": {
	                "previous": "Sebelumnya",
	                "next": "Seterusnya",
			...
			"aoColumnDefs":[{          
                ...
                "mRender": function ( value, type, full )  {
					...
					button_b = '<button type="button" class="btn btn-icon" title="Paparan Tindakan Susulan Pelaporan i-Ramal" onclick="load_view_ts_imuhibbah(\'' + full.id + '\');"><i class="fa fa-search"></i></button>';
	...
	function paparan_imuhibbah(id){
		window.location.href = "{{ route('rt-sm22.paparan_pelaporan_imuhibbah_ts_p','') }}"+"/"+id;
	}
